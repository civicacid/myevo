                    ...
                    while (_positionInBag != slots + 1 && clicked != howMany)
                    {
                        ...
                        if (itemOb != null)
                        {
                            ...
                            try
                            {
                                ...
                                if (toolTip != null)
                                {
                                    string ItemName = toolTip.GetChildObject("GameTooltipTextLeft1").GetText;
                                    int ItemCount = 0;
                                    ItemCount = 1;
                                    ...
                                    if (MailList.ShouldMail(ItemName))
                                    {
                                        if (ItemName.Equals("源质矿石") || ItemName.Equals("鞭尾草")
                                            || ItemName.Equals("燃烬草") || ItemName.Equals("燃铁矿石")
                                            || ItemName.Equals("黑曜石矿") || ItemName.Equals("心灵之花")
                                            || ItemName.Equals("暮光茉莉") || ItemName.Equals("风暴藤")
                                           )
                                        {
                                            if (ItemCount == 20)
                                            {
                                                Logging.Write("Adding: " + ItemName);
                                                Thread.Sleep(150);
